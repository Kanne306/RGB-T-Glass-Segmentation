...
import datasets.transforms_nontarget as T_non
...
def evaluate(model, criterion, postprocessors, data_loader, device, output_dir, is_rgbt=True):
    ...
    for rgb, temperature, targets, img_path in data_loader:
        ...
        final_size = (max_h, max_w)
        print(final_size,type(final_size))
        # change True to False for RGB only
        outputs = model(rgb, temperature, final_size, is_rgbt)
...
@torch.no_grad()
def inferences(model, postprocessors, input, device, is_rgbt=True):
    transform = T.Compose([
        T.Resize((480,640)),
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    normalize = T_non.Compose([
        T_non.ToTensor(),
        T_non.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    transform_orig = T_non.Compose([
            T_non.RandomResize([480], max_size=1333),
            normalize,
        ])
    model.eval()
    rgb, temperature = input
    rgb, temperature = transform_orig(Image.fromarray(rgb),Image.fromarray(temperature))

    rgb = rgb.to(device)
    temperature = temperature.to(device)
    final_size = torch.tensor([[480, 640]], device='cuda:0')
    outputs = model(rgb, temperature, (480,640), is_rgbt)
    results = postprocessors['mask'](outputs, final_size,final_size)
    result = results[0].squeeze().cpu().numpy()
